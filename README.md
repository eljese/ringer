...
Route with per-task `"engine": "agy"`. `model_default` is `"gemini-3.7-flash-high"`; override per task with `"model"` set to any key listed by `agy models`. `agy -p` exits cleanly with stdin closed. **Sandbox is the default and the only safe option** (`agy --sandbox` always appears in `args_template`); the default mode is `accept-edits` so unattended file tasks do not stop at permission prompts. The engine's `full_access_args` flips to `--dangerously-skip-permissions` so `allow_full_access` remains the single escape hatch in the orchestrator. Use `--add-dir {workdir}` and `--add-dir {taskdir}` (NOT `--project` — `--project` is a project-ID token and does not pin filesystem writes). `{workdir}` is the candidate/review tree; `{taskdir}` is the write leaf. Extra `--add-dir` paths such as `/` are still rejected. Orchestrators running AGY from a Codex sandbox must use `bin/ringer-safe-run` — never launch `agy` or `ringer.py` directly, and never flip `allow_full_access` to recover from HOME/XDG/socket failures. `./ringer.py --runtime-root` is write/state containment only and is not the wrapper. Give Codex a `prefix_rule` for `/absolute/path/to/ringer/bin/ringer-safe-run` alone — not `bash`, `python`, `ringer.py`, `agy`, or the whole directory. Verified against agy 1.1.13 on 2026-08-14 (Linux); see `docs/AGY.md` and `docs/runtime-isolation.md`.
...
- [@eljese](https://github.com/eljese) (Jesse Salmi) — engine lanes, Codex installation support, test hardening, and fork maintenance
