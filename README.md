...
Route with per-task `"engine": "claude"`. `model_default` is `claude-sonnet-4-6` (the shipped default — Anthropic's frontier coding model as of 2026-07); override per task with `"model": "claude-opus-4-6"` or `"model": "claude-haiku-4-5"` for cost-sensitive lanes. `claude --print --verbose --output-format json` exits cleanly with stdin closed and exposes token counts (input, output, cache read/write) in the JSON result — Ringer reads those straight into the `swarm_runs` row. **Sandbox is the default and the only safe option** (`claude --sandbox`); the engine's `full_access_args` flips to `--dangerously-skip-permissions` so `allow_full_access` remains the single escape hatch in the orchestrator. Verified against the Claude Code CLI on Linux (2026-07-09); see `docs/CLAUDE-CODE-PROBE-RESULTS.md` for the harness-level evidence (sandbox flag, `--print` exit semantics, JSON token schema).
