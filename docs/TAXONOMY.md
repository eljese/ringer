...
Worked example, because this exact conflation has happened: **"Grok Build" is a harness, never a model and never a lab.** Grok Build CLI 1.0.3 serves Grok 4.6 and Grok 4.5 (lab: xAI). Composer 2.5 (lab: Cursor/Anysphere) is a historical slug rejected by 1.0.3. There is no model called "Grok Build"; a scoreboard row named after a harness is a taxonomy bug.
...
Grok 4.5 is the worked example: its canonical route is `grok:grok-4.5`, meaning Grok 4.5 by xAI through the Grok Build CLI on the OAuth plan. `grok:grok-build` is historical — a legacy CLI slug rejected by 1.0.3 that still resolves to Grok 4.5 on scoresheet rows. `opencode:openrouter/x-ai/grok-4.5` is a noncanonical route to that artifact. A historical row from it displays **Grok 4.5 | xAI | OpenCode | OpenRouter API**, marked `misrouted` and not ranked. It must never become a normal OpenCode registry model entry.
...
2. **Grok Build CLI:** Run `grok models` (or `grok --help`) and inspect the available models listing, then cross-check xAI's release notes at <https://docs.x.ai/developers/release-notes>. CLI 1.0.3 serves Grok 4.6 and Grok 4.5 by xAI. `--output-format json` emits a `modelUsage` key of `grok-4.6-build` when the manifest slug is `grok-4.6`, but that key is not a live `grok models` id, so the shipped lane does not parse it (`model_report_regex` omitted). Stamp the manifest/config slug. `grok-4.6-build` is registered as an alias of Grok 4.6 for any row that already carries the JSON key.
