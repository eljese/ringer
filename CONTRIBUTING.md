...
3. **Executed proof for every claim.** A test that runs beats a screenshot; a check that prints *why* it fails beats a silent `exit 1`. CI's required gate is `full suite (ubuntu-latest)`; macOS is an informative non-blocking compatibility check and `windows-latest` is a non-blocking harness. Platform claims must be proven by the job for that platform, not asserted. The checked-in [`.claude/autonomous-project.json`](.claude/autonomous-project.json) encodes that gate, squash-merge, and that the Codex GitHub review bot is not used on this fork.
